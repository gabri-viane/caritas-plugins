 ...
 */
package theopenhand.plugins.prodotti.window.dons.add;
 ...
 * @author gabri
 */
public class DonRegister extends AnchorPane implements DialogComponent {
    ...
    private void init() {
        try {
            ...
            URL resource = getClass().getResource("/theopenhand/plugins/prodotti/window/dons/add/DonRegister.fxml");
    ...
    private void saveProd() {
        String don_name = nameTF.getText();
        String don_desc = descTF.getText();
        if (softValidText(don_name) && softValidText(don_desc)) {
            Donatore c = new Donatore(don_name, don_desc);
            ConnectionExecutor.getInstance().executeCall(PluginRegisterProdotti.prr, 1, Donatore.class, c);
